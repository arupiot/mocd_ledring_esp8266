/*
 * MOCD LED ring lighting
 * ESP8266 + WS2812 lighting control for the Ministry of Collective Data exhibition
 * 
 * 2021 francesco.anselmo@gmail.com
 * 
 */

#define IP_ADDRESS 90 // last number of IP address
#define NODE_NAME "ledring"
#define NUM_LEDS    16
...
DEFINE_GRADIENT_PALETTE( mocd_gp ) {
  0,     255,  255,  255,   //white
...
void handleRoot() {
    digitalWrite ( led, 1 );
    String out = "<html><head><title>MOCD ledring</title></head>";
    out += "<body style='background-color:gray'><center>";
    out += "<span style='display:block; width:100%; font-size:2em; font-family:Verdana; text-align:center'>mocd LED rings lighting</span><br/>";
    ...
    out += "<a href='mocd'><span style='display:block; background-color:white; width:100%; height:3em;'>mocd Gradient</span></a><br/>";
    out += "<a href='rotating'><span style='display:block; background-color:white; width:100%; height:3em;'>Transition: rotating</span></a><br/>";
...
//  int pos = abs(beatsin8(13,0,NUM_LEDS));
...
//    leds[i] = CRGB::White;
    leds[i] = ColorFromPalette(palette, idx, 255);
    // Show the leds
    FastLED.show(); 
...
void setup ( void ) {
    ...
    server.on ( "/rotating", []() {transition=0; handleRoot();} );
    ...
    server.on ( "/mocd", []() {setColor(100); handleRoot();} );
...
void loop ( void ) {
    ...
    server.handleClient();
     // run transition
    if (transition==0) t_rotating();
    else if (transition==1) t_sinelon();
